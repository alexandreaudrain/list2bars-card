...
@customElement('list2bars')
export class List2Bars extends LitElement {
  public static async getConfigElement(): Promise<LovelaceCardEditor> {
    return document.createElement('list2bars-editor') as LovelaceCardEditor;
  }
  ...
  @property() private _config!: List2BarsConfig;
  @property() private _configArray: List2BarsConfig[] = [];
  ...
  public setConfig(config: List2BarsConfig): void {
    if (!config) {
    ...
    this._config = mergeDeep(
      {
        ...
        color: 'var(--list2bars-color, var(--primary-color))',
        columns: 1,
        direction: 'right',
        max: 100,
        min: 0,
  ...
  private _createBarArray(): TemplateResult[] {
    ...
    for (let i = 0; i < columnsArray.length; i++) {
      ...
      for (let x = 0; x < columnsArray[i]; x++) {
        ...
        switch (config.positions.name) {
          ...
          case 'inside':
            nameInside = html`
              <list2bars-name>${name}</list2bars-name>
            `;
            break;
          case 'off':
            break;
        ...
        switch (config.positions.minmax) {
          case 'outside':
            minMaxOutside = html`
              <list2bars-min>${config.min}${unitOfMeasurement}</list2bars-min>
              <list2bars-divider>/</list2bars-divider>
              <list2bars-max>${config.max}${unitOfMeasurement}</list2bars-max>
            `;
            break;
          case 'inside':
            minMaxInside = html`
              <list2bars-min class="${config.direction == 'up' ? 'min-direction-up' : 'min-direction-right'}"
                >${config.min}${unitOfMeasurement}</list2bars-min
              >
              <list2bars-divider>/</list2bars-divider>
              <list2bars-max> ${config.max}${unitOfMeasurement}</list2bars-max>
            `;
        ...
        switch (config.positions.value) {
          case 'outside':
            valueOutside = html`
              <list2bars-value class="${config.direction == 'up' ? 'value-direction-up' : 'value-direction-right'}"
                >${config.complementary ? config.max - entityState : entityState} ${unitOfMeasurement}</list2bars-value
              >
            `;
            break;
          case 'inside':
          ...
          case 'outside':
            indicatorOutside = html`
              <list2bars-indicator
                ...
                >${indicatorText}</list2bars-indicator
              >
            `;
            break;
          case 'inside':
            indicatorInside = html`
              <list2bars-indicator style="--bar-color: ${barColor};">${indicatorText}</list2bars-indicator>
            `;
            break;
          case 'off':
            break;
          ...
          >
            ...
            <list2bars-background
              style="margin: ${backgroundMargin}; height: ${barHeight}${typeof barHeight == 'number'
                ...
                : ''}; ${barWidth}"
            >
              <list2bars-backgroundbar style="--bar-color: ${barColor};"></list2bars-backgroundbar>
    ...
    for (const row of perRowArray) {
      rowArray.push(html`
        <list2bars-row style="flex-direction: ${rowFlexDirection};">${row}</list2bars-row>
      `);
    }
    return rowArray;
  }
  ...
  private _computeBarColor(value: string, index: number): string {
    ...
    if (config.severity) {
      ...
      barColor = `var(--list2bars-disabled-color, ${config.color})`;
    } else {
